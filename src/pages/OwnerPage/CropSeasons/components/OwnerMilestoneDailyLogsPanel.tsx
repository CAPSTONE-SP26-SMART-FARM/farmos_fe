...
/**
 * Owner-side mirror của `MilestoneDailyLogsPanel`. Khác biệt:
 *   - dùng `useOwnerDailyLogsByFarm` (BE: `GET /daily-log/owner/farm/:farmId`)
 *     thay vì `useManagerDailyLogsByZone`. Filter `zoneId + milestoneId` được
 *     truyền qua query params.
 *   - Tự resolve farmId qua `useOwnerGetMyFarm` (1 owner = 1 farm).
 */
export function OwnerMilestoneDailyLogsPanel({
  zoneId,
  milestoneId,
}: {
  zoneId: string;
