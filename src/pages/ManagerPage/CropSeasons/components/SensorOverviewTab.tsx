...
function AlertsTable({ alerts, isLoading }: { alerts: AlertResType[]; isLoading: boolean }) {
  if (isLoading) {
    return (
    ...
    <div className="rounded-md border overflow-hidden">
      <table className="w-full text-xs">
        ...
        <tbody className="divide-y">
          {alerts.map((a) => (
            <tr key={a.id} className={`${ALERT_SEVERITY_COLORS[a.severity] ?? ""} transition-colors`}>
        ...
        <div className="sticky top-4 space-y-2">
          <div className="flex items-center gap-1.5">
            ...
            <h4 className="text-sm font-semibold">
              Cảnh báo{" "}
              {alerts.length > 0 && <span className="text-destructive">({alerts.length})</span>}
            </h4>
          </div>
