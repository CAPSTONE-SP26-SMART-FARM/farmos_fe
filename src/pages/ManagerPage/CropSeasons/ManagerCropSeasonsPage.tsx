...
const REQUEST_STATUS_MAP: Record<string, { label: string; variant: "default" | "secondary" | "destructive" }> = {
	...
	return (
		<Dialog open={open} onOpenChange={setOpen}>
			<DialogTrigger asChild>
				<Button size="sm"><Plus className="h-4 w-4 mr-1" />Tạo mùa vụ</Button>
			</DialogTrigger>
			<DialogContent className="max-w-lg">
				...
				<form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-2">
					...
					<div className="grid grid-cols-2 gap-3">
						<Field label="Ngày trồng * (YYYY-MM-DD)" error={form.formState.errors.plantDate?.message}>
							<Input
						...
						<Field label="Ngày thu hoạch dự kiến *" error={form.formState.errors.expectedHarvestDate?.message}>
							<Input
...
function UpdateCropSeasonDialog({ season }: { season: CropSeasonType }) {
	...
	return (
		<Dialog open={open} onOpenChange={setOpen}>
			<DialogTrigger asChild>
				<Button size="sm" variant="outline"><Pencil className="h-3 w-3 mr-1" />Sửa</Button>
			</DialogTrigger>
			<DialogContent className="max-w-lg">
				<DialogHeader><DialogTitle>Cập nhật mùa vụ</DialogTitle></DialogHeader>
				<form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-2">
					<Field label="Tên cây trồng" error={form.formState.errors.cropName?.message}>
						<Input {...form.register("cropName")} autoComplete="off" />
					</Field>
					<Field label="Giống / Loại">
						<Input {...form.register("variety")} autoComplete="off" />
					</Field>
					<div className="grid grid-cols-2 gap-3">
						<Field label="Ngày trồng (YYYY-MM-DD)">
							<Input {...form.register("plantDate")} autoComplete="off" />
						</Field>
						<Field label="Ngày thu hoạch dự kiến">
							<Input {...form.register("expectedHarvestDate")} autoComplete="off" />
						</Field>
					</div>
					<div className="grid grid-cols-2 gap-3">
						<Field label="Diện tích (m²)">
							<Input type="number" {...form.register("totalAreaSqm", { valueAsNumber: true })} autoComplete="off" />
						</Field>
						<Field label="Số lượng cây">
							<Input type="number" {...form.register("plantCount", { valueAsNumber: true })} autoComplete="off" />
						</Field>
					</div>
					<Field label="Ghi chú">
						<Textarea {...form.register("notes")} rows={2} className="resize-none" />
...
function SendRequestDialog({ season }: { season: CropSeasonType }) {
	...
	return (
		<Dialog open={open} onOpenChange={setOpen}>
			<DialogTrigger asChild>
				<Button size="sm" variant={season.status === ProductionStatusName.Rejected ? "destructive" : "default"}>
			...
			<DialogContent className="max-w-md">
				<DialogHeader>
					<DialogTitle>
						{season.status === ProductionStatusName.Rejected ? "Gửi lại yêu cầu" : "Gửi yêu cầu phê duyệt"}
					</DialogTitle>
				</DialogHeader>
				<div className="space-y-4 pt-2">
					...
					<p className="text-sm text-muted-foreground">
						Gửi mùa vụ <strong>{season.cropName}</strong> lên Owner để phê duyệt.
						Sau khi gửi, mùa vụ <strong>không thể chỉnh sửa thêm</strong>.
					</p>
					<form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
						<Field label="Ghi chú cho Owner (tuỳ chọn)">
							<Textarea {...form.register("description")} rows={3} placeholder="Mô tả thêm về kế hoạch..." className="resize-none" />
						</Field>
		...
		<div className="space-y-5">
			<div className="grid grid-cols-2 gap-3 text-sm">
				{([
					...
					["Diện tích", season.totalAreaSqm ? `${season.totalAreaSqm} m²` : "—"],
...
function CropSeasonDetailDialog({ season }: { season: CropSeasonType }) {
	...
	return (
		<Dialog open={open} onOpenChange={setOpen}>
			<DialogTrigger asChild>
				<Button size="sm" variant="ghost"><Eye className="h-3 w-3 mr-1" />Chi tiết</Button>
			</DialogTrigger>
			<DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
				<DialogHeader>
					<DialogTitle>{season.cropName}{season.variety ? ` — ${season.variety}` : ""}</DialogTitle>
				</DialogHeader>
		...
		<div className="space-y-6">
			<div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
				<div>
					...
					<p className="text-muted-foreground">Lên kế hoạch, theo dõi và gửi phê duyệt mùa vụ cho Owner.</p>
				</div>
			...
			<Card>
				<CardContent className="pt-4">
					<div className="flex gap-2 items-end">
						<Field label="Zone ID (lấy từ npm run prisma:seed output)" className="flex-1">
							<Input
								value={zoneInput}
						...
						<Button
							variant="outline"
							className="mb-0.5"
							onClick={() => { setZoneId(zoneInput.trim()); setSearchParams(new URLSearchParams()); }}
						>
							Tải danh sách
						</Button>
					</div>
			...
			{zoneId && (
				<div className="flex gap-2 items-center">
					...
					{statusFilter && (
						<Button variant="ghost" size="sm" onClick={() => { const p = new URLSearchParams(searchParams); p.delete("status"); setSearchParams(p); }}>
							Xoá bộ lọc
						</Button>
					)}
				</div>
			...
			<Card>
				<CardHeader>
					<CardTitle className="text-base flex items-center gap-2">
						Danh sách mùa vụ
						{!isLoading && zoneId && (
							<span className="text-muted-foreground font-normal text-sm">({totalItems} mùa vụ)</span>
						)}
					</CardTitle>
				</CardHeader>
				<CardContent>
								...
								{!isLoading && seasons.length === 0 && (
									<TableRow>
										<TableCell colSpan={7} className="text-center text-muted-foreground py-12">
											{!zoneId ? "Nhập Zone ID ở trên để xem danh sách mùa vụ" : "Chưa có mùa vụ nào. Bấm \"Tạo mùa vụ\" để bắt đầu!"}
										</TableCell>
									</TableRow>
								)}
								{seasons.map((s) => (
									<TableRow key={s.id}>
										...
										<TableCell className="text-muted-foreground">{s.variety ?? "—"}</TableCell>
										<TableCell>{formatDate(s.plantDate)}</TableCell>
										<TableCell>{formatDate(s.expectedHarvestDate)}</TableCell>
										<TableCell>{s.totalAreaSqm ? `${s.totalAreaSqm} m²` : "—"}</TableCell>
						...
						<div className="flex justify-between items-center mt-4">
							...
							<ProPagination
								...
								buildHref={(p) => {
									...
									p ? params.set("page", String(p)) : params.delete("page");
